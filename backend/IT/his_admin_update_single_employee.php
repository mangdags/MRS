<?php
	...
	include('assets/inc/config.php');
		if(isset($_POST['update_doc']))
		{
			...
			$query="UPDATE his_admin SET ad_fname=?, ad_lname=?,  ad_email=?, ad_pwd=?, ad_dpic=? WHERE ad_id  = ?";
			$stmt = $mysqli->prepare($query);
			$rc=$stmt->bind_param('ssssss', $doc_fname, $doc_lname, $doc_email, $doc_pwd, $doc_dpic, $ad_id);
			$stmt->execute();
			/*
                                        ...
                                        <form method="post" enctype="multipart/form-data">
                                            <div class="form-row">
                                                <div class="form-group col-md-6">
                                                    ...
                                                    <input type="text" required="required" value="<?php echo $row->ad_fname;?>" name="doc_fname" class="form-control" id="inputEmail4" >
                                                </div>
                                                <div class="form-group col-md-6">
                                                    <label for="inputPassword4" class="col-form-label">Last Name</label>
                                                    <input required="required" type="text" value="<?php echo $row->ad_lname;?>" name="doc_lname" class="form-control"  id="inputPassword4">
                                                </div>
                                            </div>

                                            <div class="form-group">
                                                <label for="inputAddress" class="col-form-label">Email</label>
                                                <input required="required" type="email" value="<?php echo $row->ad_email;?>" class="form-control" name="doc_email" id="inputAddress">
                                            </div>
