        ...
        
        {
			...
			*/ 
			...
			if($stmt)
			{
                ...
                if ($eqp_name != $_POST['name']) {
                    $changes = 'Change lab Equipment name <br>To: '.$eqp_name.'<br> From: '.$_POST['name'].'';
                    $query = "INSERT INTO his_audit(changes, user_id, date) VALUES('$changes','$id_user','$date')";
                    $stmt = $mysqli->prepare($query);
                    $stmt->execute();
                }
                if ($eqp_vendor != $_POST['vendor']) {
                    $changes = 'Change lab Equipment named '.$eqp_name.' vendor <br>To: '.$eqp_vendor.'<br> From: '.$_POST['vendor'].'';
                    $query = "INSERT INTO his_audit(changes, user_id, date) VALUES('$changes','$id_user','$date')";
                    $stmt = $mysqli->prepare($query);
                    $stmt->execute();
                }
                if ($eqp_qty != $_POST['qty']) {
                    $changes = 'Change lab Equipment named '.$eqp_name.' quantity <br>To: '.$eqp_vendor.'<br> From: '.$_POST['qty'].'';
                    $query = "INSERT INTO his_audit(changes, user_id, date) VALUES('$changes','$id_user','$date')";
                    $stmt = $mysqli->prepare($query);
                    $stmt->execute();
                }
                if ($eqp_desc != $_POST['desc']) {
                    $changes = 'Change lab Equipment named '.$eqp_name.' description <br>To: '.$eqp_desc.'<br> From: '.$_POST['desc'].'';
                    $query = "INSERT INTO his_audit(changes, user_id, date) VALUES('$changes','$id_user','$date')";
                    $stmt = $mysqli->prepare($query);
                    $stmt->execute();
                }
                                ...
                                <div class="col-12">
                                    <div class="card">
                                        <div class="card-body">
                                            ...
                                            <!--Add Patient Form-->
                                            <form method="post">
                                                <input type="hidden" name="name" value="<?php echo $row->eqp_name;?>">
                                                <input type="hidden" name="vendor" value="<?php echo $row->eqp_vendor;?>">
                                                <input type="hidden" name="qty" value = "<?php echo $row->eqp_qty;?>">
                                                <input type="hidden" name="desc" value="<?php echo $row->eqp_desc;?>">
                                                ...
                                                <div class="form-row">
                                                    ...
                                                    <div class="form-group col-md-6" style="display:none">
                                                        ...
                                                        <input required="required" type="hidden" value="Functioning" name="eqp_status" class="form-control"  id="inputPassword4">
                                                    </div>
